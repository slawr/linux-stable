...
#define OV10635_V_START_L		0x381f
...
static int ov10635_s_crop(struct v4l2_subdev *sd, struct v4l2_crop *a)
{
...
static int ov10635_video_probe(struct i2c_client *client)
{
	...
	dev_info(&client->dev, "%s Product ID %x Manufacturer ID %x\n",
		 devname, pid, ver);
 ...
 * i2c_driver function
 */
static __devinit int ov10635_probe(struct i2c_client *client,
			const struct i2c_device_id *did)
{
	...
	priv = kzalloc(sizeof(struct ov10635_priv), GFP_KERNEL);
	if (!priv) {
		dev_err(&client->dev,
...
static __devexit int ov10635_remove(struct i2c_client *client)
{
